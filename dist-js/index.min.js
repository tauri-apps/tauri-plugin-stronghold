...
class ProcedureExecutor {
    ...
    async generateSLIP10Seed(outputLocation, sizeBytes) {
        return await _("plugin:stronghold|execute_procedure", {
            ...this.procedureArgs,
            procedure: {
                type: "SLIP10Generate",
                payload: {
...
class Store {
    ...
    async get(key) {
        return await _("plugin:stronghold|get_store_record", {
    ...
    async insert(key, value, lifetime) {
        return await _("plugin:stronghold|save_store_record", {
            snapshotPath: this.path,
            client: this.client,
            key: toBytesDto(key),
            value,
    ...
    async remove(key) {
        return await _("plugin:stronghold|remove_store_record", {
            snapshotPath: this.path,
            client: this.client,
            key: toBytesDto(key),
...
class Vault extends ProcedureExecutor {
    ...
    async insert(recordPath, secret) {
        return await _("plugin:stronghold|save_secret", {
            snapshotPath: this.path,
            client: this.client,
            vault: this.name,
    ...
    async remove(location) {
        return await _("plugin:stronghold|remove_secret", {
            snapshotPath: this.path,
            client: this.client,
            vault: this.name,
...
class Stronghold {
    ...
    static async load(path, password) {
        return await _("plugin:stronghold|initialize", {
            snapshotPath: path,
            password,
     ...
     */
    async unload() {
        return await _("plugin:stronghold|destroy", {
            snapshotPath: this.path,
        });
    }
    async loadClient(client) {
        return await _("plugin:stronghold|load_client", {
            snapshotPath: this.path,
            client: toBytesDto(client),
        }).then(() => new Client(this.path, client));
    }
    async createClient(client) {
        return await _("plugin:stronghold|create_client", {
            snapshotPath: this.path,
            client: toBytesDto(client),
        }).then(() => new Client(this.path, client));
    }
     ...
     */
    async save() {
        return await _("plugin:stronghold|save", {
            snapshotPath: this.path,
        });
    }
}
